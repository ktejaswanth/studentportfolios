  ...
  return (
    <div className="min-h-screen bg-zinc-950 text-white selection:bg-purple-500/30 overflow-x-hidden font-outfit">
      ...
      <div className="max-w-5xl mx-auto px-6 py-20 lg:py-32 space-y-36">
        {/* Hero */}
        <section id="about" className="space-y-10 min-h-[80vh] flex flex-col justify-center animate-fade-in">
          ...
          <p className="text-xl text-zinc-500 max-w-2xl font-medium leading-relaxed italic border-l-2 border-white/5 pl-8 py-2">
             {student.introduction || student.summary}
          </p>
          {student.about_me && (
            <div className="p-8 rounded-3xl bg-white/[0.02] border border-white/[0.06] text-zinc-400 text-lg leading-relaxed">
               {student.about_me}
            </div>
          )}
        </section>
        ...
        {(student.section_order || ['summary', 'experience', 'projects', 'education', 'skills']).map((section: string) => {
          if (section === 'summary') {
            return (
              <section key="about" id="about-intro" className="space-y-10 animate-fade-in py-20">
                <div className="inline-flex items-center gap-2 p-1 px-4 w-fit rounded-full bg-white/5 border border-white/10 text-[10px] font-black uppercase tracking-[0.3em] text-purple-400">
                   <Sparkles size={12} /> The Story
                </div>
                <div className="space-y-8">
                   <p className="text-3xl md:text-5xl font-black italic tracking-tighter leading-tight uppercase bg-gradient-to-r from-white to-zinc-500 bg-clip-text text-transparent">
                     {student.introduction || student.summary}
                   </p>
                   {student.about_me && (
                     <div className="p-10 rounded-[2.5rem] bg-white/[0.02] border border-white/[0.06] text-zinc-400 text-xl leading-relaxed italic border-l-8 border-purple-500 shadow-2xl">
                        {student.about_me}
                     </div>
                   )}
                </div>
          ...
          if (section === 'projects' && projects.length > 0) {
            return (
              <section key="work" id="work" className="space-y-16 py-20">
                 <SectionHeading title="Selected Works" subtitle="Projects that define me" />
                 <div className="grid md:grid-cols-2 gap-6">
                    {projects.map((proj: any, i: number) => (
                      <div key={i} className="group relative bg-white/[0.02] overflow-hidden border border-white/[0.06] rounded-[2.5rem] hover:border-purple-500/30 transition-all duration-700">
                         ...
                         <div className="relative p-12 space-y-8">
                            <div className="space-y-4">
                               <h3 className="text-4xl font-black italic uppercase group-hover:text-transparent group-hover:bg-clip-text group-hover:bg-gradient-to-r group-hover:from-purple-400 group-hover:to-orange-400 transition-all duration-500 leading-none">{proj.title}</h3>
                               <div className="flex flex-wrap gap-2">
                                 {proj.tech.split(',').map((t: string, j: number) => (
                                   <span key={j} className="text-[10px] font-bold uppercase tracking-widest text-purple-400/60 bg-purple-400/5 px-4 py-1.5 rounded-full border border-purple-400/10">{t.trim()}</span>
                                 ))}
                               </div>
                            </div>
                            <p className="text-zinc-500 text-lg leading-relaxed">{proj.description}</p>
                            {proj.link && (
                              <a href={proj.link} target="_blank" className="inline-flex items-center gap-3 text-xs font-black uppercase tracking-widest text-white/40 hover:text-purple-400 transition-all duration-300 group-hover:translate-x-4">
          ...
          if (section === 'certifications' && certifications?.length > 0) {
             return (
               <section key="certifications" className="space-y-16 py-20">
                  <SectionHeading title="Hall of Fame" subtitle="My Certifications" />
                  <div className="grid md:grid-cols-3 gap-6">
                     {certifications.map((cert: any, i: number) => (
                       <div key={i} className="p-10 rounded-3xl bg-white/[0.02] border border-white/[0.06] hover:bg-purple-500/5 hover:border-purple-500/20 transition-all group">
                          <h3 className="text-2xl font-black uppercase italic group-hover:text-purple-400 transition-colors">{cert.name}</h3>
                          <p className="text-zinc-500 text-sm mt-3 font-bold">{cert.issuer}</p>
                          <p className="text-[10px] font-black uppercase tracking-[0.3em] text-purple-400 mt-10 opacity-50">{cert.date}</p>
                       </div>
                     ))}
                  </div>
               </section>
          ...
          if (section === 'experience' && experiences.length > 0) {
            return (
              <section key="exp" className="space-y-16 py-20">
                 <SectionHeading title="Experience" subtitle="My professional journey" />
                 <div className="grid gap-6">
                    {experiences.map((exp: any, i: number) => (
                       <div key={i} className="group grid md:grid-cols-[1fr_3fr] gap-10 p-10 rounded-3xl bg-white/[0.02] border border-white/[0.06] hover:border-purple-500/20 hover:bg-white/[0.04] transition-all duration-700">
                          <div className="space-y-2">
                             <span className="text-[10px] font-black text-purple-500/60 font-mono bg-purple-500/5 px-4 py-1.5 rounded-full border border-purple-500/10 uppercase tracking-[0.2em]">{exp.duration}</span>
                             <h3 className="text-3xl font-black uppercase italic group-hover:text-purple-400 transition-colors duration-300 pt-2">{exp.company}</h3>
                          </div>
                          <div className="space-y-4">
                             <p className="text-white text-2xl font-black uppercase italic tracking-tight">{exp.role}</p>
                             <p className="text-zinc-500 text-lg leading-relaxed font-medium">{exp.description}</p>
                          </div>
                       </div>
                    ))}
                 </div>
          ...
          if (section === 'skills' && skills.length > 0) {
            return (
              <section key="skills" className="space-y-16 py-20">
                 <SectionHeading title="Arsenal" subtitle="Skills and expertise" />
                 <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
                    {skills.map((skill: any, i: number) => (
                       <div key={i} className="group p-8 rounded-3xl bg-white/[0.02] border border-white/[0.06] hover:border-purple-500/20 transition-all duration-500 text-center space-y-6">
                          <div className="relative w-20 h-20 mx-auto">
                             <svg className="w-full h-full rotate-[-90deg]">
                                <circle cx="40" cy="40" r="36" className="fill-none stroke-white/5 stroke-[4]" />
                                <circle cx="40" cy="40" r="36" className="fill-none stroke-purple-500 stroke-[4] transition-all duration-1000" style={{ strokeDasharray: '226.2', strokeDashoffset: 226.2 - (226.2 * skill.level / 100) }} />
                             </svg>
                             <div className="absolute inset-0 flex items-center justify-center text-xs font-black">{skill.level}%</div>
                          </div>
                          <h3 className="text-sm font-black uppercase tracking-widest group-hover:text-purple-400 transition-colors">{skill.name}</h3>
                       </div>
                    ))}
                 </div>
              </section>
          ...
          if (section === 'education' && education.length > 0) {
            return (
              <section key="education" className="space-y-16 py-20">
                 <SectionHeading title="Education" subtitle="Academic background" />
                 <div className="grid md:grid-cols-2 gap-8">
                    {education.map((edu: any, i: number) => (
                       <div key={i} className="p-10 rounded-[2.5rem] bg-white/[0.02] border border-white/[0.06] hover:border-orange-500/20 transition-all">
                          <h3 className="text-2xl font-black uppercase italic leading-tight">{edu.college}</h3>
                          <p className="text-orange-400/80 font-bold mt-2 uppercase tracking-wide">{edu.degree}</p>
                          <div className="mt-8 flex items-center justify-between text-[10px] font-black uppercase tracking-[0.2em] text-zinc-600">
                             <span>Class of {edu.year}</span>
                             <span className="bg-white/5 px-4 py-1.5 rounded-full border border-white/5">GPA: {edu.cgpa}</span>
                          </div>
                       </div>
                    ))}
                 </div>
          ...
          if (section === 'achievements' && student.achievements) {
             return (
               <section key="achievements" className="py-20">
                  <SectionHeading title="Impact" subtitle="My achievements" />
                  <div className="p-20 rounded-[3rem] bg-gradient-to-br from-purple-500/10 via-transparent to-orange-500/10 border border-white/10 text-center relative overflow-hidden group">
                     <div className="absolute inset-0 bg-[radial-gradient(circle_at_center,_var(--tw-gradient-stops))] from-purple-500/5 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-1000" />
                     <p className="text-4xl md:text-7xl font-black italic uppercase leading-[0.85] tracking-tighter relative z-10">
          ...
          if (section === 'hobbies' && student.hobbies) {
             return (
               <section key="hobbies" className="py-20">
                  <SectionHeading title="Hobbies" subtitle="Beyond the code" />
                  <p className="text-4xl md:text-6xl font-black italic uppercase leading-[0.85] tracking-tighter text-zinc-800 hover:text-white transition-colors duration-700">
                     {student.hobbies}
                  </p>
               </section>
             )
          ...
          if (section === 'interests' && student.interests) {
             return (
               <section key="interests" className="py-20">
                  <SectionHeading title="Interests" subtitle="Future exploration" />
                  <p className="text-4xl md:text-6xl font-black italic uppercase leading-[0.85] tracking-tighter text-zinc-800 hover:text-white transition-colors duration-700">
                     {student.interests}
                  </p>
               </section>
